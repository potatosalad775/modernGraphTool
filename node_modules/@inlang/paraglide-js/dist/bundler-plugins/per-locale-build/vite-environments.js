    ...
    return {
        ...
        async config(config, env) {
            ...
            if (config.builder?.buildApp !== undefined) {
                throw new Error("experimentalPerLocaleBuild Vite environments cannot compose with an existing builder.buildApp orchestrator. A framework must expose a public client-variant API before using this backend.");
            }
            ...
            for (const variant of plan.variants) {
                if (config.environments?.[variant.environmentName] !== undefined) {
                    throw new Error(`experimentalPerLocaleBuild cannot create Vite environment ${JSON.stringify(variant.environmentName)} because the application already defines it.`);
                }
            }
            ...
            return {
                ...
                environments: Object.fromEntries(plan.variants.map((variant) => [
        ...
        configResolved(config) {
            ...
            if (config.builder?.buildApp !== buildApp) {
                throw new Error("experimentalPerLocaleBuild Vite environments must own builder.buildApp. Another plugin replaced the locale build orchestrator.");
            }
        },
        buildStart() {
            if (!this.meta.rolldownVersion) {
                throw new Error("experimentalPerLocaleBuild Vite environments require a Rolldown-powered Vite build (Vite 8 or newer).");
            }
        },
    };
}
function createLocaleEnvironment(variant) {
    return {
        consumer: "client",
...
async function buildViteLocaleEnvironments(args) {
    const outputRoot = nodePath.resolve(args.builder.config.root, args.plan.outputDirectory);
    await Promise.all([
    ...
    for (const variant of args.plan.variants) {
        ...
        if (!environment) {
            throw new Error(`Vite did not create the locale environment ${JSON.stringify(variant.environmentName)}.`);
        }
    ...
    await nodeFs.writeFile(nodePath.join(outputRoot, VITE_LOCALE_BUILD_MANIFEST), JSON.stringify(manifest, undefined, "\t") + "\n");
}
function getRolldownOutputs(result) {
    if (Array.isArray(result)) {
        return result.filter((item) => "output" in item);
    }
    if ("output" in result)
        return [result];
    throw new Error("experimentalPerLocaleBuild Vite environments do not support watch-mode Rolldown output.");
}
function createLocaleManifest(args) {
    const outputFiles = args.outputs.flatMap((output) => output.output);
    const emittedFileNames = new Set(outputFiles.map((output) => output.fileName));
    const files = outputFiles
        ...
        const dynamicImports = partitionImports(output.dynamicImports, emittedFileNames);
        return {
            fileName: output.fileName,
            type: "chunk",
    ...
    for (const file of files) {
        for (const imported of [
            ...
            if (!fileNames.has(imported)) {
                throw new Error(`Vite emitted an open locale graph for ${JSON.stringify(args.variant.locale)}: ${file.fileName} imports missing ${imported}.`);
            }
        }
    }
    const entryPairs = [];
    ...
    const entries = Object.fromEntries(entryPairs.sort(([left], [right]) => left.localeCompare(right)));
    return {
...
function partitionImports(imports, emittedFileNames) {
    const internal = [];
    const external = [];
...
function normalizeOutputDirectory(directory) {
    ...
    if (normalized === "" ||
        ...
        normalized === ".." ||
        normalized.startsWith("../")) {
        throw new Error("experimentalPerLocaleBuild Vite environment outputDirectory must be a project-relative directory.");
    }
    return normalized;
}
function validateViteLocaleBuildSettings(settings) {
    if (!settings.baseLocale || !settings.locales.includes(settings.baseLocale)) {
        throw new Error("experimentalPerLocaleBuild Vite environments require baseLocale to be present in locales.");
    }
    if (settings.locales.length === 0 ||
        new Set(settings.locales).size !== settings.locales.length) {
        throw new Error("experimentalPerLocaleBuild Vite environments require at least one unique locale.");
    }
}
//# sourceMappingURL=vite-environments.js.map