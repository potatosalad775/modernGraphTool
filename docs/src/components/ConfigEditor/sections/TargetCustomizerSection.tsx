...
import type {
	FilterForm,
	FilterPresetForm,
	InitialTargetFilterForm
} from '@site/src/utils/configDefaults';
...
export default function TargetCustomizerSection() {
	...
	const removeInitialFilter = (index: number) => {
		set(
			...
			initialFilters.filter((_, i) => i !== index)
		);
	};
	...
	return (
		<AccordionSection
			...
			onToggleEnabled={(v) =>
				dispatch({ type: 'SET_FIELD', path: ['TARGET_CUSTOMIZER_ENABLED'], value: v })
			}
		>
