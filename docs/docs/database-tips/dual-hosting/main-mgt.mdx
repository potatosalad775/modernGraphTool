...
:::info[CDN-mode installs: leave `BASE_PATH` unset]
Because modernGraphTool is served from the **root** of its (sub)domain in this layout, leave `CDN_MODE.BASE_PATH` unset (or commented out) in `config.js`. If you later move mGT into a subfolder, you'll need to set `BASE_PATH` to match — see the [CDN deployment guide](../../guide-for-admins/deployment/cdn#why-base_path-is-almost-always-required) for the full explanation.
:::
