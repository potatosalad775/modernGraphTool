...
HpTF entries visualise **variance** between several related measurements — for example a sweep of slightly different fit positions (Center, Front, Back, Up, Down), or measurement rigs — as a shaded fill envelope overlaid on the graph, with optional per-sample curves. The deviation fill replaces the normal measurement curve; an HpTF entry is effectively one phone whose graph *is* the envelope itself. The samples can represent anything you want compared — HpTF is not limited to fit positions.

- `hptf.files` (Array of Strings, Required): The base file names for each deviation sample. The tool loads `{name} L.txt` and `{name} R.txt` for each entry.
- `hptf.labels` (Array of Strings, Optional): Human-readable labels shown in the HpTF toggle UI. Defaults to the `files` values if omitted.
- `hptf.description` (String, Optional): A short note displayed alongside the fill envelope describing what varies, e.g. `"(Fit Position)"`, `"(Rig Variance)"`, or `"(Insertion Depth)"`.
- `hptf.fillOnly` (Boolean, Optional, default `true`): When `true`, only the fill envelope is rendered. Set to `false` if you want users to toggle individual per-sample deviation curves in the UI.
...
:::note[`file` / `suffix` are ignored]
When a phone entry contains an `hptf` block, any `file` or `suffix` fields on the same entry are **discarded by the parser** — the entry is always treated as HpTF-only and the deviation envelope takes over. If you want a regular measurement alongside a device, add it as a separate `phone_book.json` entry.
:::
