...
### v2.2.0
- Feat: 기기 등급을 `phone_book.json` 대신 게시된 스프레드시트(squigRanking 시트 또는 임의의 CSV)에서 읽어올 수 있게 되었습니다. 시트만 수정하면 재배포 없이 등급이 갱신됩니다. 등급 스케일이 정의된 경우 색상 배지로 표시됩니다. 새로운 `RANKING` 섹션에서 설정하며, 기존 `RANKING_URL`을 대체합니다(해당 키도 계속 동작합니다). 자세한 내용은 [RANKING 문서](./guide-for-admins/customize-page.mdx#ranking) 참조.
