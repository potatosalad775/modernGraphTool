...
Click the button that matches what you're uploading, pick a `.txt` file from your computer, and it appears on the graph as if it were a local device or target. The accepted format is the standard CrinGraph-style two-column `frequency\tSPL` text file — the same format modernGraphTool and CrinGraph both use for their on-disk device databases. You can use this to:
