...
modernGraphTool is a web-based tool for visualizing and comparing frequency response (FR) measurements of headphones, IEMs, and speakers. Load measurements, overlay multiple curves, apply targets and EQ, and share results — all from the browser.
...
If you're coming from modernGraphTool v1, see the [migration guide](./migrating-v1-to-v2) for what's changed. For background on why this project exists, see [Why modernGraphTool?](./why-moderngraphtool).

## Features

Everything is built-in and configurable through a single [`config.js`](./guide-for-admins/customize-page) file. For detailed documentation on individual features, see the [Features](./category/features) section.
...
- **Multi-sample measurements** — Display individual samples or their average when a device has multiple measurement runs
- **HpTF deviation** — Visualize headphone transfer function sample-to-sample deviation as fill area, individual curves, or both
...
- **[Parametric EQ](./features/equalizer)** — Full-featured EQ panel with multiple filter types (PEQ, LSQ, HSQ), adjustable frequency/gain/Q, and preamp control
- **AutoEQ** — Automatically generate EQ profiles to match a selected target curve
...
- **[Device PEQ bridge](./features/device-peq)** — Push EQ settings directly to 20+ compatible hardware devices via USB HID, Serial, BLE, or Network

### Target Customization

- **[Target Customizer](./features/target-customizer)** — Adjust target curves with parametric filters (Tilt, Bass, Treble, Ear Gain, and more) using slider controls
...
- **[Preference Bound](./features/preference-bound)** — Overlay a shaded upper/lower preference range on the graph, based on research data
...
- **[Frequency Tutorial](./features/frequency-tutorial)** — Interactive overlay showing frequency band names and descriptions, helpful for beginners

### squig.link Integration

When hosted on `*.squig.link` domains, modernGraphTool activates additional features for the [squig.link](https://squig.link) network. See [squig.link Integration](./features/squiglink-integration) for details.
