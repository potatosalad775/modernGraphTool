...
If you only want to customize `config.js`, `theme.css`, `data/`, or `assets/`, you don't need to build from source — use [CDN Deployment](./cdn) or [Pre-built Release](./prebuilt) instead.
...
:::warning[Don't import from defaults/ or static/ as modules]
...
The build output goes to `dist/`. From there, deployment is identical to [Pre-built Release](./prebuilt) — upload the folder to your web server, make sure the SPA fallback is configured (`.htaccess` for Apache, or equivalent for other servers).
