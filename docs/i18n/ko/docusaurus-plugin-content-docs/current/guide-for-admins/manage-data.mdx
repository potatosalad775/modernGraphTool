...
:::note[variants가 우선합니다]
`variants`가 있으면 해당 phone의 `file` / `suffix` / `prefix` / `samples` / `hptfs`
키는 **무시됩니다**. 병합이 아니라 우선순위이므로, 한 phone은 둘 중 한 방식으로만
선언하세요.
:::
...
선언하지 않은 모든 variant에 적용됩니다. 모든 기기를 다섯 번씩 측정하는 데이터베이스
라면 항목마다 `samples`를 적을 필요가 전혀 없습니다. 브랜드 단위로는 `name` 옆에
`defaultSamples`를 두어 그 브랜드의 기기에만 다른 기본값을 줄 수 있습니다.
[`SAMPLES`](./customize-page#samples) 설정 섹션을 참고하세요.
:::

##### 이전 방식의 샘플 키

같은 것을 선언하던 두 가지 예전 형식이 있고, 이들은 **계속 읽힙니다**. 교차 사이트
검색이 다른 사이트의 CrinGraph 형식 phone book을 크롤링하기 때문에 영구적으로
지원됩니다. 마이그레이션할 필요는 없으며, 새 형식은 표현할 수 있는 구성이 더 많을
뿐입니다.

**phone 레벨 `samples: N`** — 번호 측정을 `file[]`의 모든 variant에 적용합니다.

```json
{ "name": ["Multi Sample"], "file": ["Multi Sample"], "samples": 3 }
```

`"variants": [{ "file": "Multi Sample", "samples": 3 }]`과 동등합니다. 이 형식의
한계가 `variants`가 존재하는 이유입니다. phone의 모든 variant가 같은 측정 횟수를
공유해야 하고, 라벨도 음영도 쓸 수 없습니다.

**`hptfs[]`** — 항목 하나가 편차 세트 하나이고, 각각이 독립된 variant가 됩니다.
...
`samples.files`와 `display: ["avg", "fill"]`을 가진 `variants` 항목과 동등합니다.
`fillOnly`가 `false`이면 여기에 `"curves"`가 추가됩니다.

:::note[여러 형식 함께 쓰기]
한 phone 항목의 일반 `file` / `suffix` variant, phone 레벨 `samples`, `hptfs`는 한
평면 목록에서 **각각 독립된 variant**로 취급되며 서로 조합되지 않습니다. `file`
variant 2개와 `hptfs` 항목 2개가 있으면 variant 선택기에 4개 항목이 표시됩니다.
:::

:::tip[CrinGraph와 함께 호스팅하기]
라벨, 음영, 개별 곡선 설정은 **번호 파일 위에 얹히는 부가 메타데이터**입니다.
`count`를 쓰는 데이터베이스는 CrinGraph의 `num_samples` 사이트가 기대하는
`{file} L{n}.txt` 배치를 그대로 유지하므로, 같은 측정 폴더로 두 도구를 모두 서비스할
수 있습니다. CrinGraph는 추가 키를 무시하고, modernGraphTool은 그것을 사용합니다.
:::
