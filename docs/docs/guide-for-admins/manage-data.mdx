...
:::tip[GUI editor]
Prefer a form? Use the [phone_book.json Editor](/phone-book-editor) to import, edit, and export `phone_book.json` without touching raw JSON. It includes inline explanations for every phone type (Simple, Detailed, Variations, Multi-Sample, HpTF).
:::
...
#### Multi-Sample Entries
A multi-sample entry groups several numbered measurement runs of the same device into one phone. By default the UI displays the **average** of all runs, and users can toggle individual samples via the Sample picker (which appears automatically when `samples > 1`).

- `samples` (Number, Required): The number of measurement runs. The tool loads files named `{file} L{n}.txt` and `{file} R{n}.txt` for `n = 1..samples`.
- Works with all regular phone-object keys (`reviewLink`, `price`, `shopLink`, etc.).
- The measurement files still live in `data/phones/` like any other entry.
...
The example above loads six files:
...
If a variant with `samples: N` is missing the numbered files `{file} L1.txt` / `{file} R1.txt`, the tool falls back to the unnumbered pair `{file} L.txt` / `{file} R.txt` and treats the variant as a single-sample measurement. This lets sibling variants in the same `file` array freely mix numbered and unnumbered pairs — a variant without numbered samples will still load without error.
:::

:::tip[Default display]
You can change whether multi-sample phones open as an averaged single curve or as all individual samples by adjusting `MULTI_SAMPLE.DEFAULT_DISPLAY` in `config.js`. See the [`MULTI_SAMPLE`](./customize-page#multi_sample) config section.
:::

#### HpTF (Headphone Transfer Function) Entries
HpTF entries visualise **variance** between several related measurements — for example a sweep of slightly different fit positions (Center, Front, Back, Up, Down), or measurement rigs — as a shaded fill envelope overlaid on the graph, with optional per-sample curves. Each HpTF set is effectively one device variant whose graph *is* the envelope itself. The samples can represent anything you want compared — HpTF is not limited to fit positions.

A single device can declare **multiple** HpTF sets via the `hptfs` array — for example one set per ear pad (Leather / Suede), or one per fit depth. Each `hptfs` entry becomes its own independent variant in the device selector.

- `hptfs` (Array of Objects, Required): One or more HpTF measurement sets. Each entry has the following keys:
  - `files` (Array of Strings, Required): The base file names for each deviation sample. The tool loads `{name} L.txt` and `{name} R.txt` for each entry.
  - `labels` (Array of Strings, Optional): Human-readable labels shown in the HpTF toggle UI. Defaults to the `files` values if omitted.
  - `suffix` (String, Optional): Variant suffix shown in the device selector dropdown (e.g. `"Leather Pad"`). When a device has multiple `hptfs` entries, this is what the user picks between. Optional when there is only one entry.
  - `description` (String, Optional): A short note displayed alongside the fill envelope describing what varies, e.g. `"(Fit Position)"`, `"(Rig Variance)"`, or `"(Insertion Depth)"`.
  - `fillOnly` (Boolean, Optional, default `true`): When `true`, only the fill envelope is rendered. Set to `false` if you want users to toggle individual per-sample deviation curves in the UI.
- Measurement files live in `data/phones/` like any other entry.
...
**Multiple HpTF sets (e.g. different ear pads):**
...
The example above produces two independent variants — *HpTF Multi Pad Leather Pad* and *HpTF Multi Pad Suede Pad* — that the user can switch between via the variant dropdown in the device selector.

:::note[Combining with regular measurements]
You can also declare `file` / `suffix` variants and `hptfs` on the same phone entry. They are treated as **independent variants** in a single flat list, not combined — a phone with two `file` variants and two `hptfs` entries produces four separate entries in the variant selector (e.g. `Stock`, `Modded`, `Leather Pad`, `Suede Pad`).
:::

:::tip[Default display]
Fill opacity and the default display mode (`fill`, `fill+curves`, `curves`, `none`) are configurable via the [`HPTF`](./customize-page#hptf) config section.
:::
