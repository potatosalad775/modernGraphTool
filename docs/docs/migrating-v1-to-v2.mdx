...
4. **Migrate `config.js`** — Copy your v1 `config.js` settings into the v2 `config.js` template. The core settings are the same; review the [new sections](./whats-new-in-v2#new-config-sections) (`SQUIGLINK`, `TARGET_CUSTOMIZER`, `PREFERENCE_BOUND`, `SAMPLES`, `CDN_MODE`) and adjust as needed. You can use the [Config Editor](/config-generator) to easily migrate and review all settings.
