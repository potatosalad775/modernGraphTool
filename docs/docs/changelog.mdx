...
- Feat: Sample sets can carry a label per measurement run. Labels appear in the sample picker and on the on-graph label, e.g. `HD 600 Leather Pad (Center, R)`.
...
- Fix: An individual sample curve is no longer always the L/R average — each run now has its own L, R and AVG toggles.
...
- Change: The phone-level `samples: N` key keeps working and stays supported — it shares the CrinGraph-compatible shape of the rest of a phone entry.
