...
- `SQUIGLINK_FALLBACK`: When no index is reachable and the site runs on squig.link, crawl each squig.link site's `phone_book.json` instead. Slower — one request per database.
