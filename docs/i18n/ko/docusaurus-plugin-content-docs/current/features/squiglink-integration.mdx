...
squig.link 연동은 modernGraphTool을 squig.link 생태계와 자연스럽게 연결합니다. 크로스 사이트 검색, 쇼핑 링크, 분석 추적, 스폰서 배너를 함께 제공합니다.
...
- **`ENABLE_CROSS_SITE_SEARCH`** — 크로스 사이트 기기 검색 켜기/끄기
...
참여 중인 squig.link 사이트 전체에서 헤드폰 측정값을 검색합니다. 결과를 누르면 해당 사이트의 그래프 도구로 바로 이동하며, 기기가 로드된 상태로 열립니다.
