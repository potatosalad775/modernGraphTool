...
Each fork became its own island. Features added in one didn't make it to others, and operators had to choose which subset of capabilities they wanted — with no way to combine them.
...
| **Target adjustment** | Fork-dependent; one target, 3–4 hardcoded filters | [Per-target adjustments, configurable filters, filter presets](./features/target-customizer) |
| **Parametric EQ** | External plugin scripts, fragmented across forks | Built-in with [AutoEQ, real-time audio preview, and interactive EQ](./features/equalizer) |
| **Device PEQ bridge** | [External plugin by jeromeof](https://github.com/jeromeof/devicePEQ) | [Imported and refactored](./features/device-peq) as a built-in component |
...
- **One file to configure** — [`config.js`](./guide-for-admins/customize-page) controls everything: initial devices, targets, normalization, watermarks, feature toggles, and more.
- **One file to theme** — [`theme.css`](./guide-for-admins/customize-page) uses CSS custom properties for graph colors, UI accent, and more. The online [Theme Generator](/theme-generator) creates it for you.
...
- **Dual-hosting** — Run modernGraphTool alongside your existing CrinGraph on the same domain if you want to [transition gradually](./database-tips/dual-hosting/main-cringraph).
...
Head to the [Setup Guide](./guide-for-admins/setup-env) to set up your environment, or jump to [Customizing the Page](./guide-for-admins/customize-page) if you already have modernGraphTool running.
:::
