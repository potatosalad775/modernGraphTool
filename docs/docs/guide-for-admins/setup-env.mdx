...
Neither is required for the [GitHub Pages](./deployment/github-pages) option, which starts from a template repository and is edited in the browser.
...
| ---------------------------------------------------- | ------------------------------ | ------------------------- | ---------------------------------- | -------------------------------------- |
| **[GitHub Pages](./deployment/github-pages)**        | Operators without hosting      | Automatic                 | Config, theme, data, HTML metadata | Handled automatically                  |
| **[CDN Deployment](./deployment/cdn)** (recommended) | Most operators                 | Automatic                 | Config, theme, data                | Requires one config line (`BASE_PATH`) |
| **[Pre-built Release](./deployment/prebuilt)**       | Controlled rollouts, airgapped | Manual (re-download)      | Config, theme, data, HTML metadata | Works out of the box                   |
| **[Building from Source](./deployment/from-source)** | Developers, forks              | Manual (git pull + build) | Everything                         | Works out of the box                   |
...
### [Building from Source](./deployment/from-source)
