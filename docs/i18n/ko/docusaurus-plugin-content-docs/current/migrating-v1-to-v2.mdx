...
4. **`config.js` 마이그레이션** — v1의 `config.js` 설정을 v2 `config.js` 템플릿에 옮깁니다. 핵심 설정은 동일하니 [새 섹션](./whats-new-in-v2#new-config-sections)(`SQUIGLINK`, `TARGET_CUSTOMIZER`, `PREFERENCE_BOUND`, `MULTI_SAMPLE`, `HPTF`, `CDN_MODE`)을 한 번 훑어보고 필요에 맞게 조정하세요. [Config Editor](/config-generator)를 쓰면 모든 설정을 한 번에 검토하고 옮길 수 있습니다.
...
v1은 사전 빌드된 배포 파일 한 가지 방식만 제공했습니다. 파일을 받아 업로드하면 끝이었죠. v2는 여기에 두 가지 옵션이 더 추가되었습니다.
