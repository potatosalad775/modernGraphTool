---
sidebar_position: 5
description: A walkthrough of the panels, layouts, and navigation that make up the modernGraphTool interface.
---
...
|---|---|---|
| **Device** | Browse, search, and load headphones. | [Loading devices](./06-loading-devices.mdx) |
| **Graph** | Pick a target, adjust the graph itself, manage loaded curves. | [Graph controls](./08-graph-controls.mdx) and [Working with curves](./07-working-with-curves.mdx) |
| **Equalizer** | Build an EQ profile, preview it, optionally send it to hardware. | [Equalizing audio](./10-equalizing.mdx) |
| **Misc** | Theme, language, site description, app info. | [Appearance and language](./12-appearance-and-language.mdx) |
...
Ready for the details? Start with [Loading devices](./06-loading-devices.mdx), where the actual data comes from.
