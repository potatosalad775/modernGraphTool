...
:::warning[CDN-mode installs: set `BASE_PATH`]
If you installed modernGraphTool with the [**CDN deployment**](../../guide-for-admins/deployment/cdn) method (`cdn-index.html` renamed to `index.html`), you also need to set `CDN_MODE.BASE_PATH` in `config.js` to match the subfolder you placed mGT in. For the `/mGT/` layout above:
...
Without this, the mGT home page will load but share links will 404. See [Why `BASE_PATH` is almost always required](../../guide-for-admins/deployment/cdn#why-base_path-is-almost-always-required) for the full explanation. This step is not needed for Pre-built Release installs.
:::
