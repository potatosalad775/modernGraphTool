<script lang="ts">
	...
	function handleBaselineClick(uuid: string) {
		...
		// Three-mode cycle for targets with adjustments: off → withAdjustment → withoutAdjustment → off
		const currentUUID = graphStore.baselineUUID;
		const currentMode = graphStore.baselineMode;

		if (currentUUID !== uuid) {
			// Different UUID or no baseline: start with withAdjustment
		...
		} else if (currentMode === 'withAdjustment') {
			// Switch to withoutAdjustment
			graphEngine.updateBaselineData(true, { uuid });
			graphStore.baselineMode = 'withoutAdjustment';
		} else {
			// withoutAdjustment → off
				...
				<div class="-mt-1 mb-2 -mr-1 flex items-center justify-end gap-1">
					...
					>
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-4 w-4">
							{#if isBaseline && graphStore.baselineMode === 'withAdjustment'}
							...
							{:else if isBaseline && graphStore.baselineMode === 'withoutAdjustment'}
