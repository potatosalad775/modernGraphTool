...
function renderKindFields(
	...
	switch (phone.kind) {
		case 'simple':
			return <SimplePhoneFields phone={phone} onPatch={onPatch} />;
		case 'detailed':
			return <DetailedPhoneFields phone={phone} onPatch={onPatch} />;
		case 'variations':
...
export default function PhoneEditor({
	...
	return (
		<div className={styles.pbPhoneCard}>
			...
			>
				<span className={`${styles.pbBrandChevron} ${isExpanded ? styles.pbBrandChevronOpen : ''}`}>
					&#9654;
				</span>
				...
				<div className={styles.pbPhoneBody}>
					<div className={styles.pbKindSelectRow}>
						...
						<div className={styles.pbKindDescription}>
							...
							<a href={DOCS_BASE + kindOption.docAnchor} target="_blank" rel="noopener noreferrer">
								Learn more
							</a>
						</div>
					</div>
