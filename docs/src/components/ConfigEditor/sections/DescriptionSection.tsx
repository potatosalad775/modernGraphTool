...
export default function DescriptionSection() {
	...
	const renderItems = (
		items: DescriptionItemForm[],
		onChange: (items: DescriptionItemForm[]) => void
	) => {
						...
						<div className={styles.ceFieldGroup}>
							...
							{item.TYPE === 'HTML' ? (
								...
								<input
									...
									onChange={(e) => updateItem(i, { CONTENT: e.target.value })}
									placeholder={
										item.TYPE === 'IMAGE' ? './assets/images/info.png' : 'Description text'
									}
								/>
							)}
						</div>
	...
	return (
		...
		>
			<I18nWrapper basePath={['DESCRIPTION']} state={state.DESCRIPTION} renderItems={renderItems} />
		</AccordionSection>
	);
}
