...
export function parseCrinGraphConfig(source: string): Record<string, any> {
	try {
		...
		const knownVars = [
			...
			'extraEQBandsMax',
			'targets',
			'preference_bounds_name',
			'preference_bounds_dir',
			'preference_bounds_startup',
			'allowSquigDownload',
			'PHONE_BOOK',
			'default_y_scale',
			'default_DF_name',
			...
			'tiltableTargets',
			'compTargets',
			'compensateMeasurementByDefault',
			'default_comp_target',
			'showCustomDFAdjustmentButton',
			'showCustomDFAdjustmentButtonOnDesktop',
			'allowCreatorSupport',
			'enableNonConfidenceIntervalTutorial',
	...
	if (extensions && extensions.length > 0) {
		...
		if (sq?.ENABLED && sq.CONFIG) {
			v2.SQUIGLINK = {
				ENABLED: true,
				DEBUG: false,
				ANALYTICS_MEASUREMENT_IDS: sq.CONFIG.ANALYTICS_GTM_ID ? [sq.CONFIG.ANALYTICS_GTM_ID] : [],
...
export function convertCrinGraphToV2(crin: Record<string, any>): ConversionResult {
	const warnings: string[] = [];
	const v2: Record<string, any> = {};
	...
	const state: ConfigFormState = {
		...
		VISUALIZATION: {
			...
			LABEL: raw.VISUALIZATION?.LABEL
				? {
						...
						...raw.VISUALIZATION.LABEL,
						POSITION: {
							...defaults.VISUALIZATION.LABEL.POSITION,
							...(raw.VISUALIZATION.LABEL?.POSITION ?? {})
						}
					}
				: { ...defaults.VISUALIZATION.LABEL },
			BASELINE_LABEL: raw.VISUALIZATION?.BASELINE_LABEL
				? {
						...
						...raw.VISUALIZATION.BASELINE_LABEL,
						POSITION: {
							...defaults.VISUALIZATION.BASELINE_LABEL.POSITION,
							...(raw.VISUALIZATION.BASELINE_LABEL?.POSITION ?? {})
						}
					}
				: { ...defaults.VISUALIZATION.BASELINE_LABEL },
		...
		INTERFACE: {
			PREFERRED_DARK_MODE_THEME:
				raw.INTERFACE?.PREFERRED_DARK_MODE_THEME ?? defaults.INTERFACE.PREFERRED_DARK_MODE_THEME,
			ALLOW_REMOVING_PHONE_FROM_SELECTOR:
				raw.INTERFACE?.ALLOW_REMOVING_PHONE_FROM_SELECTOR ??
				defaults.INTERFACE.ALLOW_REMOVING_PHONE_FROM_SELECTOR,
			SWITCH_PHONE_PANEL_ON_BRAND_CLICK:
				raw.INTERFACE?.SWITCH_PHONE_PANEL_ON_BRAND_CLICK ??
				defaults.INTERFACE.SWITCH_PHONE_PANEL_ON_BRAND_CLICK,
			TARGET: {
				ALLOW_MULTIPLE_LINE_PER_TYPE:
					raw.INTERFACE?.TARGET?.ALLOW_MULTIPLE_LINE_PER_TYPE ??
					defaults.INTERFACE.TARGET.ALLOW_MULTIPLE_LINE_PER_TYPE,
				OMIT_TARGET_SUFFIX:
					raw.INTERFACE?.TARGET?.OMIT_TARGET_SUFFIX ?? defaults.INTERFACE.TARGET.OMIT_TARGET_SUFFIX,
				COLLAPSE_TARGET_LIST_ON_INITIAL:
					raw.INTERFACE?.TARGET?.COLLAPSE_TARGET_LIST_ON_INITIAL ??
					defaults.INTERFACE.TARGET.COLLAPSE_TARGET_LIST_ON_INITIAL
			},
			HIDE_DEV_DONATE_BUTTON:
				raw.INTERFACE?.HIDE_DEV_DONATE_BUTTON ?? defaults.INTERFACE.HIDE_DEV_DONATE_BUTTON
		},
		URL: {
		...
		LANGUAGE: {
			...
			ENABLE_SYSTEM_LANG_DETECTION:
				raw.LANGUAGE?.ENABLE_SYSTEM_LANG_DETECTION ?? defaults.LANGUAGE.ENABLE_SYSTEM_LANG_DETECTION
		},
		PATH: {
		...
		TRACE_STYLING: {
			PHONE_TRACE_THICKNESS:
				raw.TRACE_STYLING?.PHONE_TRACE_THICKNESS ?? defaults.TRACE_STYLING.PHONE_TRACE_THICKNESS,
			TARGET_TRACE_THICKNESS:
		...
		TOPBAR: {
			...
			LINK_LIST: toI18nArrayState<TopbarLinkForm>(
				raw.TOPBAR?.LINK_LIST ?? defaults.TOPBAR.LINK_LIST
			)
		},
		PREFERENCE_BOUND_ENABLED: !!raw.PREFERENCE_BOUND,
		PREFERENCE_BOUND: {
			ENABLE_BOUND_ON_INITIAL_LOAD:
				raw.PREFERENCE_BOUND?.ENABLE_BOUND_ON_INITIAL_LOAD ??
				defaults.PREFERENCE_BOUND.ENABLE_BOUND_ON_INITIAL_LOAD,
			BASE_DF_TARGET_FILE:
				raw.PREFERENCE_BOUND?.BASE_DF_TARGET_FILE ?? defaults.PREFERENCE_BOUND.BASE_DF_TARGET_FILE,
		...
		TARGET_CUSTOMIZER: {
			CUSTOMIZABLE_TARGETS:
				raw.TARGET_CUSTOMIZER?.CUSTOMIZABLE_TARGETS ??
				defaults.TARGET_CUSTOMIZER.CUSTOMIZABLE_TARGETS,
			FILTERS: (raw.TARGET_CUSTOMIZER?.FILTERS ?? defaults.TARGET_CUSTOMIZER.FILTERS).map(
				(f: any): FilterForm => ({
					id: f.id ?? '',
					name: f.name ?? '',
					type: f.type ?? 'PK',
		...
		SQUIGLINK: {
			...
			ANALYTICS_MEASUREMENT_IDS:
				raw.SQUIGLINK?.ANALYTICS_MEASUREMENT_IDS ?? defaults.SQUIGLINK.ANALYTICS_MEASUREMENT_IDS,
			...
			ENABLE_CROSS_SITE_SEARCH:
				raw.SQUIGLINK?.ENABLE_CROSS_SITE_SEARCH ?? defaults.SQUIGLINK.ENABLE_CROSS_SITE_SEARCH,
