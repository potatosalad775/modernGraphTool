...
- **Source builders**: Editable source files are in the `static/` folder. The HTML entry point is `src/app.html`. After building (`npm run build`), everything is output to `dist/`.
:::
...
```
Controls the display of HpTF (Headphone Transfer Function) rig deviation data.
