...
:::info[Base target location]
The base DF target file is resolved from the same folder your other target curves live in, defined by `PATH.TARGET_MEASUREMENT` in `config.js`. See the [`PATH`](../guide-for-admins/customize-page#path) config section.
:::
