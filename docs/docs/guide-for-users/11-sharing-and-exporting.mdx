...
- The target curve if one is loaded
...
If you'd rather a live link than a static image, use Share instead — the recipient gets the interactive version with all the controls still working.
...
See [Equalizing audio](./10-equalizing.mdx) and [Equalizer](../features/equalizer) for the workflow.
