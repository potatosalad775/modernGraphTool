...
HpTF 항목은 측정값 주변의 **위치별 편차** — 보통 약간씩 다른 착용 위치를 스윕한 측정 (Center, Front, Back, Up, Down) — 를 그래프에 겹쳐진 음영 포락선으로 시각화하며, 선택적으로 샘플별 커브를 함께 표시합니다. 편차 음영이 일반 측정 커브를 대체하므로, HpTF 항목은 그래프 자체가 포락선인 하나의 phone이 됩니다.
...
- `hptf.description` (문자열, 선택 사항): 음영 포락선과 함께 표시되는 짧은 메모 (예: `"(Positional Variance)"`).
