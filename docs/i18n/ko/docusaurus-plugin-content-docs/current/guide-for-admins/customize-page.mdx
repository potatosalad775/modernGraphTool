...
주로 `index.html`, `theme.css`, `config.js` 파일을 편집하여 페이지를 개인화합니다 — 단순한 것부터 복잡한 것 순서입니다.

:::info[파일 위치]
- **사전 빌드 릴리스 사용자**: 이 파일들은 압축 해제된 릴리스 폴더의 루트에 있습니다.
- **소스 빌드 사용자**: 기본 설정과 데이터 파일은 `defaults/` 폴더에 있습니다. HTML 진입점은 `src/app.html`입니다. 빌드(`npm run build`) 후 모든 결과물은 `dist/`로 출력됩니다.
:::

## 페이지 메타데이터 변경 (`index.html`) {#changing-page-metadata-indexhtml}

`index.html` 파일은 modernGraphTool의 기본 HTML 구조와 메타데이터를 정의합니다 — 페이지 제목, 설명, 미리보기 이미지, 파비콘, 그리고 브라우저와 소셜 미디어 스크래퍼가 읽는 기타 `<head>` 필드들입니다.

:::info[파일 위치]
- **사전 빌드 릴리스**: 릴리스 폴더의 `index.html`을 직접 편집하세요.
- **소스 빌드 사용자**: `src/app.html` (SvelteKit 템플릿)을 편집하세요. 빌드 후 이 템플릿으로부터 `dist/index.html`이 생성됩니다.
:::

:::warning[주의]
HTML 파일은 HTML 문법을 따릅니다. 수정 후 문법 오류가 없는지 반드시 확인하세요.
:::
...
`theme.css` 파일은 modernGraphTool의 시각 테마를 구동하는 CSS 커스텀 속성(변수)을 담고 있습니다 — D3.js 주파수 응답 그래프와 주변 UI 크롬 모두에 적용됩니다. 모든 변수는 라이트 모드와 다크 모드 사이를 자동으로 전환합니다.

:::tip[시각적으로 테마 생성하기]
docs 사이트에는 실시간 **[테마 생성기](/theme-generator)**가 포함되어 있어, 색상을 선택하고 그래프와 UI를 라이트/다크 모드 양쪽으로 미리 보며 바로 적용할 수 있는 `theme.css` 파일을 다운로드할 수 있습니다. 아래 CSS 변수를 직접 편집하고 싶지 않다면 이 도구를 사용하세요.
:::

:::tip[팁]
`theme.css` 파일은 배포 루트에서 `index.html`과 같은 위치에 두어야 합니다.
:::

### 그래프 변수

이 변수들은 Tailwind 클래스를 사용할 수 없는 SVG 렌더링을 구동합니다:
...
UI 크롬 — 배경, 텍스트, 버튼, 액센트 — 은 DaisyUI 스타일의 기본 팔레트로 구동됩니다. 모든 변수는 라이트(`:root`)와 다크(`.dark`) 값을 가집니다:
...
각 `--color-*` 역할에는 대응되는 `--color-*-content` 변수가 있어 그 위에 렌더링되는 전경(텍스트/아이콘) 색상을 지정합니다. 덕분에 기본 색을 바꿀 때 대비가 자동으로 유지됩니다.

:::info[색상 형식]
값들은 [OKLCH 색공간](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value/oklch)을 사용합니다. 원한다면 `rgb()`, `hsl()`, hex 값도 사용할 수 있지만, OKLCH가 팔레트 전체에 걸친 지각적 밝기를 더 예측 가능하게 만들어 줍니다.
:::

## 기본 설정 변경 (`config.js`) {#changing-basic-settings-configjs}

:::tip[시각적 에디터를 선호하신다면?]
docs 사이트에는 실시간 **[Config Editor](/config-generator)**가 포함되어 있어, 직접 편집하지 않고도 유효한 `config.js`를 구성하고 내보낼 수 있습니다. 아래에 설명된 모든 필드를 다룹니다 — 이 레퍼런스 페이지는 각 필드가 무엇을 하는지 이해하거나 기존 파일을 직접 수정하고 싶을 때 여전히 유용합니다.
:::

`config.js` 파일은 modernGraphTool의 동작과 관련된 다양한 설정을 담고 있습니다. 텍스트 에디터로 파일을 열어 필요한 부분을 수정할 수 있습니다.

:::warning[주의]
`config.js` 파일은 JavaScript 문법을 따릅니다. 문자열은 따옴표(`"` 또는 `'`)로 감싸고, 각 설정 항목 끝에는 쉼표(`,`)가 필요합니다 (마지막 항목 제외). 수정 후 문법 오류가 없는지 반드시 확인하세요.
:::
...
사용자 인터페이스 관련 설정을 지정합니다.
  ...
  Target 선택기 관련 설정을 지정합니다.
    ...
    타입별로 Target 목록을 여러 줄로 표시할지 여부를 설정합니다.
    - `true`, `false` 중 선택.
    ##### `OMIT_TARGET_SUFFIX`
...
- `MAJOR_VERSION`: 앱을 특정 메이저 버전에 고정합니다 (예: `2`는 v2.x.x 내에서 자동 업데이트).
- `BASE`: 커스텀 CDN 기본 URL. CDN 자산을 직접 호스팅하는 고급 사용자만 변경합니다.
- `BASE_PATH`: 배포 하위 경로 (예: `"/headphones"`), 끝 슬래시는 붙이지 않습니다. **(서브)도메인 루트가 아닌 모든 배포에 필수입니다.** `example.com/headphones/` 또는 `username.squig.link/headphones/`에 있는 사이트의 경우 `BASE_PATH: "/headphones"`로 설정하세요. 사이트가 (서브)도메인의 루트 — 예: `example.com/` 또는 `username.squig.link/` — 에 제공될 때만 설정하지 않은 채로 둡니다. 설정하지 않으면 홈 페이지는 정상적으로 로드되지만 공유 링크 딥 라우트가 모두 404가 됩니다. 사실상 항상 필요한 이유에 대한 전체 설명은 [CDN 배포 가이드](./deployment/cdn#why-base_path-is-almost-always-required)를 참고하세요.
- `VERSIONS_URL`: `versions.json`의 전체 URL. 기본적으로 로더는 `BASE`에서 `raw.githubusercontent.com` URL을 유도해 GitHub에서 직접 `versions.json`을 가져옵니다 — jsDelivr를 통하지 **않습니다**. 이는 의도된 것입니다: jsDelivr의 엣지 캐시는 오래된 `versions.json` 응답을 몇 시간 동안 고정할 수 있어 새 릴리스가 사용자에게 도달하는 것을 지연시킵니다. `versions.json`은 이 지연이 문제가 되는 유일한 파일이므로 로더가 이를 우회합니다. `cdn` 브랜치의 자체 포크를 호스팅하거나 GitHub이 아닌 출처에서 제공하는 경우에만 `VERSIONS_URL`을 재정의하세요.

:::info[표준 배포]
[사전 빌드 릴리스](./deployment/prebuilt) 방식(전체 `dist/` 폴더 업로드)을 사용하는 경우 이 섹션은 필요하지 않습니다. 이 섹션은 `cdn-index.html`을 사용하는 [CDN 배포](./deployment/cdn)에만 관련이 있습니다.
:::
...
<Tabs>
  <TabItem value="TEXT" label="TEXT">
    ...
    - `CONTENT` : 워터마크에 표시될 텍스트를 설정합니다.
      ...
      - 2개 이상의 문자열이 지정되면 첫 로드 시 무작위로 선택된 문자열이 표시됩니다.
    - `LOCATION` : 워터마크 위치를 설정합니다.
      - `TOP_LEFT`, `TOP_RIGHT`, `BOTTOM_LEFT`, `BOTTOM_RIGHT` 중 선택.
    - `SIZE` : 워터마크 크기 (px 단위).
    - `FONT_FAMILY` : 워터마크 폰트.
    - `FONT_WEIGHT` : 워터마크 폰트 굵기 (100 ~ 900 중 백 단위 숫자).
    - `COLOR` : 워터마크 색상 (HEX 코드).
  ...
  <TabItem value="IMAGE" label="IMAGE">
    ...
    - `CONTENT` : 워터마크에 표시될 이미지 파일 경로를 설정합니다.
      ...
      - 2개 이상의 이미지 경로가 지정되면 첫 로드 시 무작위로 선택된 이미지가 표시됩니다.
    - `LOCATION` : 워터마크 위치를 설정합니다.
      - `TOP_LEFT`, `TOP_RIGHT`, `BOTTOM_LEFT`, `BOTTOM_RIGHT` 중 선택.
    - `SIZE` : 워터마크 크기 (px 단위).
    - `POSITION` : 워터마크 위치 세부 조정.
...
```
기기에 여러 개의 측정 샘플이 있을 때 다중 샘플 측정 데이터의 표시 방식을 제어합니다.
- `DEFAULT_DISPLAY`: 다중 샘플 데이터의 기본 표시 모드를 설정합니다.
  - `"average"` — 모든 샘플의 평균을 표시 (기본값).
  - `"all"` — 모든 개별 샘플 커브를 표시.
...
```
HpTF(Headphone Transfer Function) 샘플 편차 데이터의 표시 방식을 제어합니다.
- `DEFAULT_DISPLAY`: HpTF 편차의 기본 표시 모드를 설정합니다.
  - `"fill"` — 음영 처리된 편차 영역만 표시.
  - `"fill+curves"` — 음영 영역과 개별 편차 커브를 모두 표시.
  - `"curves"` — 개별 편차 커브만 표시.
  - `"none"` — HpTF 편차를 표시하지 않음.
...
```
그래프에 음영 처리된 상한/하한 선호 범위를 표시하는 선호 범위(Preference Bound) 오버레이를 설정합니다.
...
:::info[데이터 파일]
선호 범위가 표시되려면 `data/` 폴더에 bound 데이터 파일(`Bounds U.txt` / `Bounds D.txt`)이 있어야 합니다.
:::
...
```
지정된 타겟 커브에 대해 타겟별 필터 조정을 가능하게 하는 Target Customizer를 설정합니다.
- `CUSTOMIZABLE_TARGETS`: 커스터마이즈 가능한 타겟 파일 이름의 배열입니다.
- `FILTERS`: 사용 가능한 필터 정의. 각 필터는 다음 항목을 가집니다:
  ...
  - `freq` — 중심/코너 주파수 (Hz).
  - `q` — Q 팩터 (대역폭).
- `FILTER_PRESET`: 드롭다운에서 선택 가능한 필터 프리셋 구성.
  - `name` — 프리셋 표시 이름.
  - `filter` — 필터 ID를 게인 값에 매핑하는 객체.
- `INITIAL_TARGET_FILTERS`: 초기 페이지 로드 시 자동으로 적용되는 필터.
  - `name` — 필터를 적용할 타겟 이름.
  - `filter` — 필터 ID를 게인 값에 매핑하는 객체.
...
```
squig.link 연동을 설정합니다. 이 기능은 `*.squig.link` 도메인에서 호스팅될 때만 활성화됩니다 (`DEBUG`가 활성화된 경우 제외).
- `ENABLED`: squig.link 연동의 마스터 토글입니다.
- `DEBUG`: `true`로 설정하면 호스팅 도메인과 관계없이 squig.link 기능을 활성화합니다 (개발/테스트용).
...
`config.js` 내 일부 항목은 영어 이외의 언어로 작성될 수 있습니다.
- 다국어 지원을 활성화하려면 `config.js` 파일의 `LANGUAGE` 항목을 수정해야 합니다.
...
다국어 지원 항목의 경우, 기본 언어(영어)로 작성된 객체는 `default` 필드에, 다른 언어로 작성된 객체는 `i18n` 필드에 작성해야 합니다.
- `i18n` 필드에는 `LANGUAGE` 항목 내 `LANGUAGE_LIST`에 등록된 언어 코드와 일치하는 객체가 있어야 합니다.
