...
4. **`config.js` 마이그레이션** — v1의 `config.js` 설정을 v2 `config.js` 템플릿에 옮깁니다. 핵심 설정은 동일하니 [새 섹션](./whats-new-in-v2#new-config-sections)(`SQUIGLINK`, `TARGET_CUSTOMIZER`, `PREFERENCE_BOUND`, `SAMPLES`, `CDN_MODE`)을 한 번 훑어보고 필요에 맞게 조정하세요. [Config Editor](/config-generator)를 쓰면 모든 설정을 한 번에 검토하고 옮길 수 있습니다.
