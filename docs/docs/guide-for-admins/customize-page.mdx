...
You will primarily personalize the page by modifying the `config.js`, `index.html`, and `theme.css` files.
...
```
Configures the squig.link integration. These features are only active when hosted on `*.squig.link` domains (unless `DEBUG` is enabled).
- `ENABLED`: Master toggle for squig.link integration.
- `DEBUG`: When `true`, enables squig.link features regardless of the hosting domain (for development/testing).
...
The `index.html` file defines the basic HTML structure and metadata for modernGraphTool.
...
The `theme.css` file contains CSS custom properties (variables) that control the appearance of the D3.js frequency response graph.

You can manually modify this CSS file to customize graph colors and styling. The [Theme Generator](/theme-generator) can also help you generate a customized `theme.css` file.
...
:::info[Note]
The `theme.css` file controls graph-specific styling only. The overall UI uses Tailwind CSS with built-in light/dark mode support. Full UI color customization is planned for a future release.
:::

:::tip[Tip]
The `theme.css` file should be co-located with `index.html` at the root location.
:::
