...
The tradeoff is that your site needs to reach jsDelivr at load time, and you don't directly control _when_ the update ships. If either of those matters to you, see [Pre-built Release](./prebuilt) instead.
...
:::tip[No web server?]
[GitHub Pages](./github-pages) is this same deployment mode with GitHub providing the hosting, set up from a template repository entirely in your browser. It also resolves `BASE_PATH` automatically, so the step-4 caveat below doesn't apply there.
:::
...
:::warning[Trade-off]
Self-hosting the CDN assets defeats the "no manual updates" selling point of CDN mode. You now have two things to maintain: your site _and_ your mirror. For most operators, the [Pre-built Release](./prebuilt) path is simpler.
:::
