...
HpTF entries visualise **positional variance** around a measurement — typically a sweep of slightly different fit positions (Center, Front, Back, Up, Down) — as a shaded fill envelope overlaid on the graph, with optional per-sample curves. The deviation fill replaces the normal measurement curve; an HpTF entry is effectively one phone whose graph *is* the envelope itself.
...
- `hptf.description` (String, Optional): A short note displayed alongside the fill envelope, e.g. `"(Positional Variance)"`.
