...
[Sample Sets](./manage-data#sample-sets) for the `phone_book.json` side.
...
`SAMPLES` replaces both. The old sections are still read as a fallback, so an
existing `config.js` keeps working unchanged — but set `SAMPLES` and delete them
...
- `HPTF.DEFAULT_DISPLAY: "fill"` → `["avg", "fill"]`
- `HPTF.DEFAULT_DISPLAY: "fill+curves"` → `["avg", "curves", "fill"]`
- `HPTF.DEFAULT_DISPLAY: "curves"` → `["avg", "curves"]`
- `HPTF.DEFAULT_DISPLAY: "none"` → `[]`
