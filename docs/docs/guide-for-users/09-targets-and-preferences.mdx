---
sidebar_position: 9
description: Picking targets, customizing them with the Target Customizer, and reading the preference bound overlay.
---

# Targets and preferences

This page is the practical follow-up to [Why targets exist](./04-why-targets-exist.mdx). Now that you know what a target represents, here's how to pick one, adjust it, and read the preference bound.
...
Drag a slider and the target curve reshapes immediately. The changes are **on top of** the original target — they don't replace it. If you turn baseline on with "without adjustment" vs "with adjustment" modes (see [Working with curves](./07-working-with-curves.mdx)), you can see the effect of the adjustments in isolation.
...
:::info[This just adjusts the _view_, not the file]
...
The full feature doc with all the config options is at [Target Customizer](../features/target-customizer).
...
Toggle the preference bound from the graph toolbar (the button looks like a shaded band). When it's on:
...
The preference bound needs data files the operator has to ship, so on sites that haven't configured it the toggle button doesn't appear. See [Preference Bound](../features/preference-bound) for the full feature doc.
...
4. **Turn on inspection mode** and hover to read the dB difference at the frequencies you care about — usually around 100 Hz for bass weight, 3 kHz for ear gain, 6–10 kHz for sibilance.
5. **Optionally turn on the preference bound** to see whether the headphone's overall tilt is inside or outside the research range.
6. **Baseline the target** if you want to see the headphone's differences _directly_ as a line around zero.

From there you can decide whether the differences are changes you'd welcome (headphones are full of compromises) or changes you'd want to [EQ away](./10-equalizing.mdx).

---

Next: [Equalizing audio](./10-equalizing.mdx).
