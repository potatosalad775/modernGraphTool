...
- Customized `index.html` metadata (OG tags, analytics snippets, etc.)
- Subdirectory hosting without any extra config — it just works
...
3.  **Customize the configuration files:**
    ...
    - `data/` — measurement data and target curves (see [Managing Data](../manage-data))
...
5.  **Check the [changelog](../../changelog)** for any config schema changes that need your attention.

:::warning[Don't overwrite your data]
...
**Fix:** Verify the paths in `PATH` (`PHONE_MEASUREMENT`, `TARGET_MEASUREMENT`, `PHONE_BOOK`) point at real files. Open the browser's network tab and check which request is 404ing. See [Managing Data](../manage-data) for the expected layout.
...
**Fix:** Place both files directly in `data/` (not in a subfolder). See the [Preference Bound feature doc](../../features/preference-bound) for the file format.
