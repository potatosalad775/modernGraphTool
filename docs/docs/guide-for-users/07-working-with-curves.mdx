---
sidebar_position: 7
description: Per-curve controls — baseline, visibility, Y-offset, channels, variations, sample selection, and color.
---
...
- **Top line:** a color swatch, the device name (with a variant dropdown if the device has variants), and a cluster of action buttons on the right.
- **Bottom line:** a channel selector (for devices that have channels) and a Y-offset control.

## Color swatch

The small colored square on the left is the curve's **color on the graph**. Click it to open the [color picker](./12-appearance-and-language.mdx#per-curve-color) — a small popover with a native color field, HSL inputs, dash-pattern controls, and a randomize button. Use it when you have several curves in similar hues and want to make one stand out.
...
If you want to keep both variants visible at the same time, the current workaround is to load the device from the Device panel a second time and switch the new copy's variant — the original row keeps its variant and you end up with both on the graph.
...
**Baseline a target that you've customized** (via the [Target Customizer](./09-targets-and-preferences.mdx)): the button cycles through three states, so you can see the effect of your customization independently from the device comparison itself.
...
:::info[Baseline is a visual aid, not a data transformation]
...
### Sample selector (for multi-sample devices)

If a device has **multiple samples** (the same model measured several times) _or_ a precomputed HpTF deviation, the channel selector is replaced by a richer **Sample / Channel selector** that also lets you choose:

- A specific sample by index, or the average across all samples.
- Whether to show the HpTF deviation as a **fill band**, as **individual curves**, or both.

This is how you drill into "is this headphone a consistent performer across units or does it vary a lot". A narrow band is consistent; a wide band shifts around.
...
Targets have one extra control that devices don't: a **Target Customizer** button (sliders icon). Clicking it opens a panel with Tilt, Bass, Treble, Ear sliders plus one-click presets. It only appears for targets that the site operator has marked as customizable. See [Targets and preferences](./09-targets-and-preferences.mdx) for the workflow.
