...
export function extractName(phone: PhoneState): string {
  switch (phone.kind) {
    case 'simple':       return phone.simple?.value ?? '';
    case 'detailed':     return phone.detailed?.name ?? '';
...
export function parsePhoneBook(jsonText: string): ParseResult {
  ...
  const brands: BrandState[] = raw.map((rawBrand, brandIdx) => {
    ...
    const name =
      ...
      typeof b.brand === 'string' ? (warnings.push(`Brand #${brandIdx}: legacy "brand" key converted to "name".`), b.brand) :
      '';
    ...
    const phones = rawPhones.map((p, phoneIdx) => parsePhone(p, name || String(brandIdx), phoneIdx, warnings));
...
function parsePhone(raw: unknown, brandLabel: string, phoneIdx: number, warnings: string[]): PhoneState {
  // Simple string entry
  if (typeof raw === 'string') {
    return { id: makeId('phone'), kind: 'simple', simple: { value: raw } };
  }
  if (typeof raw !== 'object' || raw == null || Array.isArray(raw)) {
    warnings.push(`Brand "${brandLabel}" phone #${phoneIdx}: not a string or object, coerced to empty.`);
  ...
  for (const key of Object.keys(p)) {
    if (!KNOWN_PHONE_KEYS.has(key)) {
      ...
      warnings.push(`Brand "${brandLabel}" phone #${phoneIdx}: unknown key "${key}" preserved on export.`);
    }
  }
  ...
  if (typeof p.samples === 'number') {
    const file = Array.isArray(p.file) ? String(p.file[0] ?? '') : typeof p.file === 'string' ? p.file : '';
    const suffix = Array.isArray(p.suffix) ? String(p.suffix[0] ?? '') : typeof p.suffix === 'string' ? p.suffix : '';
    return withShared({
      id: makeId('phone'),
      kind: 'multiSample',
  ...
  if (p.prefix !== undefined) {
    const prefixStr = Array.isArray(p.prefix) ? String(p.prefix[0] ?? '') : typeof p.prefix === 'string' ? p.prefix : '';
    const files = Array.isArray(p.file) ? p.file.map(String) : typeof p.file === 'string' ? [p.file] : [];
    return withShared({
      id: makeId('phone'),
      kind: 'prefix',
  ...
  if (Array.isArray(p.file)) {
    ...
    const suffixes = Array.isArray(p.suffix) ? p.suffix.map(String)
      : typeof p.suffix === 'string' ? [p.suffix]
      : [];
