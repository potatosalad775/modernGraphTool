...
[GitHub Pages](./deployment/github-pages) 옵션은 둘 다 필요 없습니다. 템플릿 저장소에서 시작해 브라우저에서 편집합니다.
...
| --------------------------------------------- | -------------------- | --------------------- | -------------------------------- | ---------------------------- |
| **[GitHub Pages](./deployment/github-pages)** | 호스팅이 없는 운영자 | 자동                  | 설정·테마·데이터·HTML 메타데이터 | 자동으로 처리됨              |
| **[CDN 배포](./deployment/cdn)** (권장)       | 대부분의 운영자      | 자동                  | 설정·테마·데이터                 | 설정 한 줄(`BASE_PATH`) 필요 |
| **[사전 빌드 릴리스](./deployment/prebuilt)** | 통제된 배포 환경     | 수동(재다운로드)      | 설정·테마·데이터·HTML 메타데이터 | 추가 설정 없이 동작          |
...
사이트가 `/headphones/` 같은 하위 디렉터리에 있다면(squig.link에서 흔한 패턴입니다. 같은 서브도메인에 루트 이어폰 데이터베이스와 `/headphones/` 헤드폰 데이터베이스를 함께 두는 식입니다) `config.js`에 `CDN_MODE.BASE_PATH`를 설정해야 합니다. 한 줄짜리지만 빠뜨리기 쉬우니 [CDN 배포 가이드](./deployment/cdn#why-base_path-is-almost-always-required)에서 자세한 설명을 확인하세요.
